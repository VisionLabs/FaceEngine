...
if __name__ == "__main__":
    ...
    err, face = detector.detectOne(
        image,
        image.getRect(),
        fe.DetectionType(fe.DT_LANDMARKS5 | fe.DT_BBOX)
    )
    ...
    if not face.isValid():
        print("Detection failed! Face is not found.")
        exit(-1)

    print("Face rect: ", face.detection.getRect())
    ...
    detectionSize = detection.getRect().width
