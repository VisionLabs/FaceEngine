...
class TestFaceEngineRect(unittest.TestCase):
    ...
    def test_AttributeEstimator(self):
        ...
        err_batch, list_result, aggregate_result = attributeEstimator.estimate([image, image], attribute_result)
        self.assertTrue(err_batch.isOk)
        for result in list_result:
            self.assertEqual(attribute_result.gender_opt.value())
            self.assertEqual(attribute_result.genderScore_opt.value(), result.genderScore_opt.value())
            self.assertEqual(attribute_result.ethnicity_opt.value().caucasian, result.genderScore_opt.value())
            self.assertEqual(attribute_result.age_opt.value(), result.genderScore_opt.value())
            # batch with each of image
            self.assertEqual(aggregate_result.gender_opt.value())
            self.assertEqual(aggregate_result.genderScore_opt.value(), result.genderScore_opt.value())
            self.assertEqual(aggregate_result.ethnicity_opt.value().caucasian, result.genderScore_opt.value())
            self.assertEqual(aggregate_result.age_opt.value(), result.genderScore_opt.value())
