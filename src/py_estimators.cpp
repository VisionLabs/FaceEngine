	...
	py::class_<fsdk::ILivenessFPREstimatorPtr>(f, "ILivenessFPREstimatorPtr",
		...
		.def("estimate", [](
				...
				bool useJpegCompression) {
					...
					fsdk::Result<fsdk::FSDKError> err = est->estimate(
						image, detection, landmarks5,
						useJpegCompression, estimation
					);
					return std::make_tuple(FSDKErrorResult(err), estimation);
			},
			"Checks whether or not detection corresponds to the real person.\n"
			"\tArgs\n"
			"\t\tparam1 (image): Image, format must be R8G8B8.\n"
			"\t\tparam2 (detection): Detection to proceed.\n"
			"\t\tparam3 (landmarks): Landmarks5 to proceed.\n"
			"\t\tparam4 (bool): Whether Jpeg compression should be used or not.\n"
			"\tReturns:\n"
